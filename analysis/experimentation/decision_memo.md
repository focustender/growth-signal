...
The backlog's second hypothesis (a portal re-entry prompt at 21 days of inactivity — the Sample Kit Builder already built in this project) has a simpler, single-threshold design (≥5% of prompted accounts convert within 30 days) rather than a holdout comparison, so it doesn't hit this same power problem in the same way. Its actual threshold still isn't evaluated here, since the prototype isn't receiving real customer traffic yet (it's live on a Shopify dev store, not production commerce) — there's no usage data to check a real threshold against. The follow-up pass this note called for happened: `docs/reactivation-flow.md` tested the mechanism itself against the live store and found (then fixed) a real bug — zero inventory on the companion SKUs — that would have made the reactivation flow fail silently on first real use, before any threshold could ever be measured.
