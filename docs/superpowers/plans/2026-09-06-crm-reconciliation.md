...
4. For each of the 85 `cross_system_hubspot_rows` from Step 1, build a `salesforce_record` dict `{Email, Name, Company, Status, CreatedDate, Phone}` (`Name` = `f"{First Name} {Last Name}"` from the real HubSpot row) deliberately drifted from that same real row: apply one drift function chosen deterministically (a *fifth* `random.Random(42)` instance) from: name-casing/typo, company-suffix add/remove ("Inc"/"LLC"), omit `Phone` entirely, shift `CreatedDate` (derived from the row's `Trade Signal Signup Date`) by 1-5 days. Set `Status` to `"Closed - Converted"` for every cross-system entry, unconditionally. This is not a guess: live-verified during planning via the HubSpot MCP connector, every sampled trade/commercial contact's real `lifecyclestage` property currently reads `"lead"` — the original 750-contact import (`docs/hubspot-setup.md`) only set 5 named custom properties and never touched HubSpot's own standard `lifecyclestage` field, so it's still sitting at HubSpot's default months later. `"lead"` and `"Closed - Converted"` map to different canonical stages (`"lead"` vs. `"customer"`) no matter which of the 85 rows this runs against, so the mismatch is real and guaranteed without needing per-row logic — the story genuinely is "HubSpot's lifecycle stage was never updated for this segment at all, while Salesforce shows these leads have actually closed."
